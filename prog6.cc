...
int main()
{
  ...
  BinaryFileRecord *myRec = new BinaryFileRecord();
  ...
  */
  ...
  // read records
  for (unsigned int x = 0; x < myHeader->numRecords; x++) {
	// read next
	binInfile.read((char *) myRec, sizeof(BinaryFileRecord));
	sprintf(currentLine, "strlen: %u", myRec->strLength);
	setCDKMatrixCell(myMatrix, x+2, 1, currentLine);
	sprintf(currentLine, "%s", myRec->stringBuffer);
	setCDKMatrixCell(myMatrix, x+2, 2, currentLine);
  }
